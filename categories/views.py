...
from categories.serializers import (
    CategoriesRootSerializer,
    SimpleCategoriesSerializer,
...
class CategoriesRoot(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if (
            "pk" in self.kwargs
            and Category.objects.filter(pk=self.kwargs["pk"]).exists()
        ):
...
class CategoriesLevel(viewsets.ModelViewSet):
    ...
    def find_level(self, queryset, level):
        ...
        for curr_child in queryset:
            self.find_level(queryset=curr_child.children.all(), level=level - 1)
    ...
    def get_queryset(self):
        ...
        self.find_level(
            queryset=Category.objects.filter(parent__isnull=True), level=level
        )
...
class CategoriesIslands(viewsets.ReadOnlyModelViewSet):
    ...
    def get_island_ids(self, obj, curr_island=None):
        if not curr_island:
            curr_island = set()
        for curr_node in (
            set(obj.similar_to.all()) - curr_island - self.visited_combinations
        ):
            curr_island.add(obj)
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response(
            [{"members": [obj.name for obj in temp_island]} for temp_island in result]
        )
