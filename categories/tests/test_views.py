...
class CategoriesByParentTestCase(CoreTest):
    def test_parent(self):
        returned_json = self.get_response_json(
            reverse("children-list", kwargs={"parent": 1})
        )
        category2 = {
            "id": 2,
            "name": "Second object",
            ...
            "similar_categories": [{"name": "First object"}],
        }
...
class CategoriesIslandsTestCase(CoreTest):
    def test_get_islands(self):
        ...
        self.assertEqual(
            returned_json, [{"members": ["First object", "Second object"]}]
        )
...
class CategoriesLevelTestCase(CoreTest):
    def test_level_1(self):
        ...
        returned_json = self.get_response_json(
            reverse("levels-list", kwargs={"level": 1})
        )
        category1 = {
            "id": 1,
            "name": "First object",
            "description": "Some description that doesn't matter",
            "image": returned_json[0]["image"],
            "similar_categories": [{"name": "Second object"}],
        }
        category3 = {
            "id": 3,
            "name": "third object",
        ...
        returned_json = self.get_response_json(
            reverse("levels-list", kwargs={"level": 2})
        )
        category2 = {
            "id": 2,
            "name": "Second object",
...
class CategoryRootTestCase(CoreTest):
    def test_all_roots(self):
        ...
        returned_json = self.get_response_json(
            reverse("roots-detail", kwargs={"pk": 1})
        )
        self.assertEqual(returned_json, root1)

        returned_json = self.get_response_json(
            reverse("roots-detail", kwargs={"pk": 3})
        )
...
class CategoriesSimilarityTestCase(CoreTest):
    ...
    def test_add_similar_categories(self):
        ...
        self.client.patch(
            reverse("similarity-detail", kwargs={"pk": 1}),
            data={
                "similar_to": [3],
            },
...
class SimpleCategoriesTestCase(CoreTest):
    ...
    def test_post_new_category(self):
        ...
        self.client.post(
            reverse("categories-list"), data=obj_data, format="multipart/form-data"
        )
        returned_json = self.get_response_json(
            reverse("categories-detail", kwargs={"pk": 3})
        )
        category3 = {
            "id": 3,
            "name": "test_name",
            "description": "test_description",
            "image": returned_json["image"],
            "similar_categories": [{"name": "First object"}, {"name": "Second object"}],
        }
